...
img = pygame.image.load('images/Helicopter.png')
def score(count):
	font = pygame.font.Font('freesansbold.ttf', 20)
	text = font.render('Score: '+str(count), True, white)
	surface.blit(text, (0,0))
def blocks(x_block, y_block, block_width, block_height, gap, colorChoice): 
	pygame.draw.rect(surface, colorChoice, [x_block, y_block, block_width,
		block_height])
	pygame.draw.rect(surface, colorChoice, [x_block, y_block+block_height+gap,
...
def run_game():
	...
	while not game_over:
		...
		if 8 <= current_score < 13:
			block_move = 12
			gap = imageHeight*5.4
		if 13 <= current_score < 21:
			block_move = 13
			gap = imageHeight*5.2
		if 21 <= current_score < 34:
			block_move = 14
			gap = imageHeight*5.0
		if 34 <= current_score < 55:
			block_move = 15
			gap = imageHeight*4.8
		if 55 <= current_score < 89:
			block_move = 16
			gap = imageHeight*4.6
		pygame.display.update()
		clock.tick(60)
